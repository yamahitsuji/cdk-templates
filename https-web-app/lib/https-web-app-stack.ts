...
export class HttpsWebAppStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    super(scope, id, props)

    const rootHostedZoneId = new cdk.CfnParameter(this, 'rootHZId', {
      type: 'String',
      description: 'ID of Route53 root hosted zone',
    })
    const rootZoneName = new cdk.CfnParameter(this, 'rootHZName', {
      type: 'String',
      description: 'Name of Route53 root hosted zone',
    })
    const subDomainPrefix = new cdk.CfnParameter(this, 'subDomainPrefix', {
      type: 'String',
      description: 'Subdomain name',
    })

    const rootHZ = route53.HostedZone.fromHostedZoneAttributes(
      this,
      'rootHostedZone',
      {
        hostedZoneId: rootHostedZoneId.valueAsString,
        zoneName: rootZoneName.valueAsString,
      }
    )

    const subDomainName = `${subDomainPrefix.valueAsString}.${rootHZ.zoneName}`
    const subHZ = new route53.HostedZone(this, 'subHostedZone', {
      zoneName: subDomainName,
    })

    if (!subHZ.hostedZoneNameServers) {
      new Error("devHZ don't have ns records.")
      return
    }
    new route53.NsRecord(this, 'subNSRecord', {
      zone: rootHZ,
      recordName: subDomainName,
      values: subHZ.hostedZoneNameServers,
    })
    ...
    // setup application ec2 instances
    ...
    const instanceTargets = instances.map(
    ...
    const cert = new acm.Certificate(this, 'Certificate', {
      ...
      validation: acm.CertificateValidation.fromDns(subHZ),
    })

    // add listener accepting https request on load balancer
    ...
    // redirect http to https on load balancer
    lb.addRedirect({
